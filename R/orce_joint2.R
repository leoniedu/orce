#' Otimização conjunta masculino/feminino — espaço único de agências
#'
#' Variante de [orce_joint()] que usa [orce_model_milp_joint2()]: um único
#' espaço de índices `j` para todas as agências, com bloqueio implícito via
#' capacidade (`wm[j] <= n_masculino[j]`). Requer uma única chamada a
#' [.orce_costs()] em vez de duas. Retorna estrutura idêntica a [orce_joint()].
#'
#' @inheritParams orce_joint
#' @return Lista com `alocacao`, `res_masculino`, `res_feminino` e `res_base`.
#' @export
orce_joint2 <- function(
    ucs,
    agencias,
    distancias_ucs,
