...
params_pof_0  <- list(ucs=amostra_uf,
           ...
           ucs_por_funcionario=24,
           remuneracao_funcionario=0,
           n_funcionarios_min=2, max_diarias_funcionario=40
           )
...
estrategias_pof <- bind_rows(
  ...
  tibble(params=list(modifyList(params_pof_1, 
                                list(remuneracao_funcionario=1500*12))), descricao='custo de treinamento + salario de apms'),
  tibble(params=list(modifyList(params_pof_1, 
                                list(
...
estrategias_pof_sum <- 
  ...
  mutate(custo_total=custo_deslocamento+custo_fixo+custo_total_funcionarios)%>%
...
estrategias_pof_sum%>%
  ...
  select(descricao, n_agencias, custo_total, custo_total_funcionarios, custo_diarias, custo_combustivel)%>%
  arrange(-custo_total)%>%
  gt::gt()%>%
...
amostra_uf <- amostra_mestra%>%
  ...
  rename(uc=upa)%>%
  count(agencia_codigo=as.character(codigo_agencia_sugerida), uc, name="viagens")%>%
  mutate(
    dias_coleta = 5)#%>%ungroup()%>%slice_sample(n=100)

n_trimestres <- amostra_mestra%>%ungroup%>%count(ano, trimestre)%>%nrow
n_meses <- n_trimestres*3
...
agencias_uf <-  amostra_uf%>%
  ungroup%>%
  count(agencia_codigo, name="n_ucs_jurisdicao")%>%
  mutate(max_uc_agencia=Inf, custo_fixo=0)
...
params_pnadc_0  <- list(ucs=amostra_uf,
                        ...
                        kml = 10,
                        valor_diaria = 335,
                        ## um trimestre de coleta
                        ucs_por_funcionario=4*n_meses,
                        max_diarias_funcionario=7*n_meses,
                        dias_treinamento = 0,
                        agencias_treinadas = NULL,
                        agencias_treinamento  = NULL,
                        distancias_ucs=distancias_upas,
...
estrategias_pnadc <- bind_rows(
  ...
  tibble(params=list(modifyList(params_pnadc_0, 
                                ## 1500 por mes por apm
                                list(
                                  remuneracao_funcionario=4500
                                  #, gap_limit=2
                                  ))), 
         descricao='remuneracao apm 1500 p/mes'),
  tibble(params=list(modifyList(params_pnadc_0, 
                                ## 3 meses, 1500 por mes por apm
                                list(
                                  remuneracao_funcionario=4500,
                                  agencias=agencias_uf%>%mutate(custo_fixo=30000),
                                  gap_limit=2))), 
         descricao='remuneracao apm 1500 p/mes/2 Técnicos 10000 p/ mes total custo fixo'),
  tibble(params=list(modifyList(params_pnadc_0, 
                                list(
                                  agencias=agencias_uf%>%
                                    mutate(max_uc_agencia=pmax(50, n_ucs_jurisdicao), custo_fixo=30000),
                                  remuneracao_funcionario=4500
                                  #, gap_limit=2
                                  ))), 
         descricao='remuneracao apm 1500 p/mes/2 Técnicos 10000 p/ mes custo fixo/limite 50 UCs por agência')
)%>%
