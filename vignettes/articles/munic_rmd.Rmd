...
report_plans <- function(r) {
  r1 <- r$resultado_agencias_otimo%>%left_join(agencias_bdo)%>%select(agencia_nome, n_upas=n_ucs, total_diarias, custo_diarias, custo_combustivel, distancia_total_km)
  r2 <- r$resultado_agencias_jurisdicao%>%left_join(agencias_bdo)%>%select(agencia_nome, n_upas=n_ucs, total_diarias, custo_diarias, custo_combustivel, distancia_total_km)
  
  rr <- r1%>%full_join(r2, by="agencia_nome", suffix=c("_otimo","_jurisdicao"))%>%
    select(agencia_nome, sort(names(.)))
  
  gt::gt(rr%>%sf::st_drop_geometry() )%>%gt::grand_summary_rows(fns=list(fn='sum', label="Total Superintendência"), columns = where(is.numeric),fmt = ~fmt_nums(.x, decimal_num = 0, decimal_currency = 0))%>%print_gt(decimal_num = 0, decimal_currency = 0)
}
```

# Introdução

O pacote orce é uma ferramenta poderosa projetada para otimizar a alocação de Unidades de Coleta (UCs), como setores censitários ou estabelecimentos de saúde, às agências do IBGE. Essa alocação eficiente é fundamental para garantir a coleta de dados de forma econômica e eficaz em pesquisas e censos de grande escala, como o Censo Demográfico.

Imagine o desafio de coletar dados em milhares de domicílios espalhados por um vasto território. O orce entra em ação para ajudar a definir a melhor estratégia, garantindo que cada equipe do IBGE visite os locais certos, minimizando o tempo de deslocamento e os custos envolvidos.
...
ufs <- ufs%>%mutate(uf_nome=to_title_case(uf_nome, locale = "pt"))
...
mnow%>%
  ...
  transmute(municipio_nome, agencia_nome=to_title_case(agencia_nome, locale="pt"), custo_diarias, custo_combustivel)%>%
  sf::st_drop_geometry()%>%
...
res_ufs_df <- bind_rows(res_ufs)%>%filter(name=="total")%>%left_join(ufs)
...
params_pof_dist_80 <- params_pof
params_pof_dist_80$dist_diaria_km <- 80

r_dist_80 <- do.call(what = alocar_ucs, args = params_pof_dist_80)

report_plans(r_dist_80)
