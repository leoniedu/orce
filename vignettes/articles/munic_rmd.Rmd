...
estrategias_pof <- bind_rows(
  ...
  tibble(params=list(modifyList(params_pof_0, 
                                list(dias_treinamento = 11, # 2 * 5,5 diárias
                                     agencias_treinamento  = c('292740800'),
                                     distancias_agencias=distancias_agencias_osrm))), 
         descricao='custo de treinamento'),
  tibble(params=list(modifyList(params_pof_0, 
                                list(dias_treinamento = 11, # 2 * 5,5 diárias
                                     agencias_treinamento  = c('292740800'),
  ...
  tibble(params=list(modifyList(params_pof_0, 
                                list(dias_treinamento = 11, # 2 * 5,5 diárias
                                     agencias_treinamento  = c('292740800'),
                                     custo_fixo=18000,
                                     distancias_agencias=distancias_agencias_osrm))), descricao='custo de treinamento + salario de 1 apm'),
  tibble(params=list(modifyList(params_pof_0, 
                                list(dias_treinamento = 11, # 2 * 5,5 diárias
                                     agencias_treinamento  = c('292740800'),
                                     custo_fixo=18000,
                                     max_uc_agencia=24,
                                     distancias_agencias=distancias_agencias_osrm))), descricao='custo de treinamento + salario de 1 apm / limite de 24 UPAs por agência'),
  tibble(params=list(modifyList(params_pof_0, 
                                list(dias_treinamento = 11, # 2 * 5,5 diárias
                                     agencias_treinamento  = c('292740800'),
                                     custo_fixo=0,
                                     ...
                                     distancias_agencias=distancias_agencias_osrm))), descricao='custo de treinamento /  limite de 24 UPAs por agência / adicional_troca_jurisdicao'))%>%
  mutate(resultado=purrr::map(params, ~do.call(alocar_ucs, .x)))
...
estrategias_pof_sum <- 
  ...
  mutate(custo_total=custo_deslocamento+custo_fixo_total)%>%
...
estrategias_pof_sum%>%
  ...
  select(descricao, n_agencias, custo_total, custo_fixo_total, custo_diarias, custo_combustivel)%>%
  arrange(-custo_total)%>%
  gt::gt()%>%
...
## Caso 3. Um novo modelo de coleta na PNADC

```{r}
library(dplyr)
library(orce)
## amostra mestra
amostra_mestra <- readr::read_rds("data-raw/amostra_mestra_2025_t1.rds")
...
amostra_uf <- amostra_mestra%>%
  filter(substr(upa,1,2)==ufnow$uf_codigo)%>%
  distinct(upa, ano, trimestre, .keep_all = TRUE)%>%
  rename(uc=upa)%>%
  count(agencia_codigo=as.character(codigo_agencia_sugerida), uc, name="visitas")%>%
  mutate(
    dias_coleta = 5,
    viagens = visitas)#%>%ungroup()%>%slice_sample(n=100)
...
agencias_uf <-  amostra_uf%>%
  ungroup%>%
  count(agencia_codigo)%>%
  mutate(max_uc_agencia=Inf)%>%
  ## 1 UPA por semana
  ## 3 meses =
  ## 12 upas por entrevistador
  mutate(max_uc_agencia_calc=(ceiling(n/12))*12)

## sem custo fixo nem custo de treinamento
params_pnadc_0  <- list(ucs=amostra_uf,
...
estrategias_pnadc <- bind_rows(
  tibble(params=list(params_pnadc_0), 
         descricao='sem custo fixo / sem custo de treinamento'),
  tibble(params=list(modifyList(params_pnadc_0, 
                                ## 3 meses, 1 apm a mais,  1500 por mes por apm
                                list(
                                  solver="cbc",
                                  custo_fixo=9000, agencias=agencias_uf%>%mutate(max_uc_agencia=max_uc_agencia_calc)))), 
                     descricao='solver cbc, custo fixo, max_uc_agencia=atual+1'))%>%
    mutate(resultado=purrr::map(params, ~do.call(alocar_ucs, .x))
    )
