...
params_pof_0  <- list(ucs=amostra_uf,
           ...
           ucs_por_entrevistador=24,
           remuneracao_entrevistador=0,
           n_entrevistadores_min=2, max_diarias_entrevistador=40,
           ## for symphony solver
           solver="symphony",
           gap_limit=2
           )
...
amostra_mestra <- readr::read_rds("data-raw/amostra_br_2024_01_2025_06.rds")%>%
...
carga_uf <- carga_br%>%
  ...
  ungroup#%>%filter(agencia_codigo%in%c('291360600', '291480200', '292870300', '293290300', '290490200'))
...
library(tictoc)
# tic()
# tmp <- do.call(alocar_ucs, params_carga_0)
# toc()
tic()
estrategias_carga <- bind_rows(
  ...
  tibble(params=list(modifyList(params_carga_0, 
                                ...
                                list(
                                  remuneracao_entrevistador=12*1500,
                                  agencias=agencias_uf%>%
                                    mutate(custo_fixo=12*10000)))), 
         descricao='remuneracao apm 1500 p/mes/2 Técnicos 10000 p/ mes total custo fixo'),
  tibble(params=list(modifyList(params_carga_0, 
                                list(
                                  agencias=agencias_uf%>%
                                    mutate(max_uc_agencia=pmax(60, n_ucs_jurisdicao), 
                                           custo_fixo=10000*12),
                                  n_entrevistadores_min=2,
                                  remuneracao_entrevistador=12*1500
                                  ))), 
         descricao='remuneracao apm 1500 p/mes/2 Técnicos 10000 p/ mes custo fixo/limite 60 UCs por agência, n_entrevistadores_min=2')
)%>%
  head(3)%>%
...
estrategias_carga_sum <- 
  ...
  mutate(custo_total=custo_deslocamento+custo_fixo+custo_total_entrevistadores)%>%
  select(descricao, modelo, n_agencias, custo_total, everything())
report_plans(estrategias_carga$resultado[[2]])
