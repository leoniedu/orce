...
```{r, include = FALSE}
ufnow <- ufs%>%filter(uf_sigla==params$uf_sigla_munic)
...
#params_munic$distancias_agencias <- distancias_agencias
...
# capture <- function(what, args) {
#   tfile <- tempfile()
#   zz <- file(tfile, "w")
#   sink(file=zz, type="message", split=FALSE)
#   res <- do.call(what=what, args=args)
#   sink()
#   close(zz)
#   res$output <- paste(readLines(tfile), collapse="\n")
#   unlink(tfile)
#   res
# }
...
economia_combustivel <- resumo_planos%>%filter(name=="custo_combustivel")%>%pull(reducao_pct)%>%round(1)
...
estrategias_pof <- bind_rows(
  ...
  tibble(params_pof=list(modifyList(params_pof_1, 
                                list(remuneracao_entrevistador=1500*12))), descricao='Custo de treinamento + remuneração dos apms'),
  # , tibble(params_pof=list(modifyList(params_pof_1, 
  #                               list(
