...
params:
  ...
  dias_coleta_agencia_max_carga: 600
  dias_coleta_agencia_max_munic: null 
  dias_coleta_agencia_max_pnadc: null
...
library(dplyr)
library(sf)
if(is.null(params$dias_coleta_agencia_max_munic)) {
  params$dias_coleta_agencia_max_munic <- Inf
}
...
uf_codigo_now <- 29
ufnow <- ufs%>%filter(uf_codigo==uf_codigo_now)
if(is.null(params$dias_coleta_agencia_max_pnadc)) {
  params$dias_coleta_agencia_max_pnadc <- Inf
}
...
agencias_uf <-  carga_pnadc_uf%>%
  ...
  mutate(dias_coleta_agencia_max=pmax(dias_coleta_jurisdicao, params$dias_coleta_agencia_max_pnadc), 
         custo_fixo=0)%>%
  ## tira Cipó
...
estrategias_pnadc <- bind_rows(
  ...
  tibble(params_pnadc=list(modifyList(params_pnadc_0, 
                                      list( remuneracao_entrevistador=params$remuneracao_entrevistador)
  )), 
  descricao=glue::glue('sem mínimo de entrevistador, remuneracao entrevistador {params$remuneracao_entrevistador}, sem custo fixo')),
  tibble(params_pnadc=list(modifyList(params_pnadc_0, 
                                      list(n_entrevistadores_min=2, remuneracao_entrevistador=params$remuneracao_entrevistador)
  )), 
  descricao=glue::glue('mínimo de entrevistador=2, remuneracao entrevistador {params$remuneracao_entrevistador}, sem custo fixo')),
  tibble(params_pnadc=list(modifyList(params_pnadc_0, 
                                      list(n_entrevistadores_min=3, remuneracao_entrevistador=params$remuneracao_entrevistador)
  )), 
  descricao=glue::glue('mínimo de entrevistador=3, remuneracao entrevistador {params$remuneracao_entrevistador}, sem custo fixo'))
  )%>%
  ungroup%>%#slice(1:2)%>%
