...
agencias_now <- mutate(agencias_bdo%>%filter(substr(agencia_codigo, 1, 2)==ufnow$uf_codigo),
...
toexport <- res$resultado_ucs_jurisdicao%>%
  ...
  left_join(municipios%>%select(municipio_codigo, municipio_nome), by = join_by(municipio_codigo))%>%
...
```{r}
munnow <- municipios%>%filter(municipio_codigo=='3200904')
```
...
has_pof_data <- exists("amostra_preliminar")
if (!has_pof_data) {
  message("Dados da POF 2024 BA não disponíveis. Casos 2 e 3 serão ignorados.")
...
## distancias upas
distancias_upas <- readRDS(here::here("data-raw/distancias_agencias_setores_pof_ba.rds"))%>%
  rename(uc=setor)%>%
...
relatorio_planos(estrategias_pof$resultado[[4]])
# 
estrategias_pof_sum%>%
  group_by(descricao)%>%
  select(-modelo)%>%
...
```{r}
amostra_br_path <- here::here("../orcedata/data-raw/amostra_br_2024_01_2025_06.rds")
has_amostra_br <- file.exists(amostra_br_path)
if (!has_amostra_br) {
  message("Dados da amostra mestra não disponíveis. Caso 3 será ignorado.")
  knitr::opts_chunk$set(eval = FALSE)
}
```
...
amostra_mestra <- readr::read_rds(amostra_br_path)%>%
...
## amostra pof
if (!exists("amostra_preliminar")) pof2024ba::data_cache()
...
carga_br <- bind_rows(amostra_cnefe, amostra_pnadc, amostra_pof, municipais)%>%
  filter(!is.na(uc), !is.na(agencia_codigo))%>%
...
carga_uf <- carga_br%>%
  ...
  ungroup()%>%filter(agencia_codigo%in%unique(agencia_codigo)[1:10])

## distancias uc
distancias_ucs_all <- readRDS(here::here("../orcedata/data-raw/distancias_agencias_upas_osrm_29.rds"))%>%
  rename(uc=upa)%>%
...
agencias_uf <-  carga_uf%>%
  ...
  mutate(n_entrevistadores_agencia_max=Inf, custo_fixo=0,
         diaria_valor=orce::diaria_valor_get(substr(agencia_codigo,1,7)))
...
estrategias_carga <- bind_rows(
  ...
  #ungroup%>%slice(-1)%>%
  mutate(resultado=purrr::map(params_carga, ~do.call(orce, .x)))
#toc()
