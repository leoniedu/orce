...
- **`r params2$dias_coleta_entrevistador_max`** dias de trabalho por entrevistador/ano
...
render_tables <- function(uf_sigla_now) {
  ...
  carga_pnadc_uf <- carga_pnadc%>%
    ...
    ungroup%>%
    mutate(uc=paste(upa,ano_mes),periodo=ano_mes)#%>%filter(agencia_codigo%in%unique(agencia_codigo)[1:20])
  ## distancias uc
